...
import {
  Send, Mic, Bot, User, MessageSquare, Plus, Download, Trash2, AlertCircle
} from 'lucide-react';
  ...
  const handleSendMessage = async () => {
    ...
    if (connectionStatus !== 'connected') {
      setMessages(prev => [...prev, {
        ...
        type: 'bot',
        content: "⚠️ Unable to connect to backend.",
        timestamp: new Date(),
    ...
    // Dynamically update chat title if it's still "New Chat"
  ...
  return (
    <div className="pt-16 h-screen flex bg-[#121f3d]">
      {connectionStatus !== 'connected' &&
        <div className="fixed top-16 left-0 right-0 z-50 bg-red-600/90 text-white px-4 py-2 text-center text-sm flex items-center justify-center">
          <AlertCircle className="w-4 h-4 mr-2" />Backend disconnected
        </div>
      }
      ...
      <motion.div initial={{ x: -300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-72 h-full bg-[#1B3E70] p-4 border-r border-[#2a3c5a] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="p-2 text-slate-400 hover:text-white hover:bg-[#2a3c5a] rounded-lg" onClick={handleNewChat}>
            <Plus className="w-5 h-5" />
          </motion.button>
        </div>
        <div className="flex-grow overflow-y-auto pr-2 space-y-2">
          {conversations.map(chat => (
            <motion.button key={chat.id} onClick={() => setActiveChatId(chat.id)} whileHover={{ x: 5 }}
              className={`w-full text-left p-3 rounded-lg flex items-center space-x-3 ${activeChatId === chat.id ? 'bg-[#2998ff]' : 'text-slate-300 hover:bg-[#2a3c5a]'}`}>
        ...
        <div className="bg-[#182a45]/80 backdrop-blur-sm border-b border-[#2a3c5a] p-4">
          <div className="max-w-4xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleDeleteChat} className="flex items-center space-x-2 px-4 py-2 bg-red-600/50 text-red-300 rounded-lg hover:bg-red-600 hover:text-white">
                <Trash2 className="w-4 h-4" /><span>Delete</span>
              </motion.button>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={exportChat} className="flex items-center space-x-2 px-4 py-2 bg-[#2a3c5a] text-slate-300 rounded-lg hover:bg-slate-600/50 hover:text-cyan-400">
              ...
              {messages.filter(m => m.chat_id === activeChatId).map(msg => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex items-start space-x-3 max-w-3xl ${msg.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    <motion.div whileHover={{ scale: 1.1 }} className={`p-2 rounded-full ${msg.type === 'user' ? 'bg-[#2998ff]' : 'bg-[#5f84a4]'}`}>
                    ...
                    <div className={`flex flex-col ${msg.type === 'user' ? 'items-end' : 'items-start'}`}>
                      ...
                      {msg.debug_info && showDebugInfo && <details className="mt-2 text-xs">
                        <summary className="text-slate-500 cursor-pointer">Debug Info</summary>
                        <pre className="mt-1 p-2 bg-slate-800 rounded text-slate-400 overflow-auto max-w-lg">{JSON.stringify(msg.debug_info, null, 2)}</pre>
                      </details>}
                    </div>
                  </div>
                </motion.div>
